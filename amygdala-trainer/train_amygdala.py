...
CONSISTENCY CHECK (SPEC.md, protocol step 5): before trusting any head, run a self-consistency / test-retest
check -- annotate the SAME clip twice (different days) and compute CCC between the two label traces (reuse ccc()
on y1 vs y2). A low test-retest CCC means the signal is NOT learnable; do not trust the held-out fit.
TODO (optional, non-breaking): add `--consistency a1.json a2.json` to compute and print that retest CCC and exit,
without touching the single-valence-output training path below.
"""
...
def main():
    ...
    ap.add_argument("--media", required=True); ap.add_argument("--annotation", required=True)
