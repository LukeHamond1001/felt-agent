...
- **Action.** A diffusion / flow-matching action expert for high-DoF continuous control, plus an optional voice head. This is the VLA frontier (GR00T, π0, OpenVLA).
...
This is the heart of the architecture and the hardest part, and it is **not a good/bad classifier.** The amygdala is the system that biases **attention** (what you notice, what you can't look away from), generates the sense of **beauty** (what draws you), and produces **inclinations** — the pre-verbal pulls toward and away that you cannot put into words.

That unverbalizability is the whole problem. You cannot label "the particular way this draws my attention" or "this specific beauty," so you cannot build a dataset of it, so you cannot supervise it the way you train everything else. A vision-language model captures only the *verbalizable shadow* — semantic similarity to affect words — not the pull that casts it. **That is why the amygdala is so weird to train, and why a picture-labeling proxy was always going to be a category error.** It is `the wall`. The closest grounding obtainable is a single emotionally-exceptional human mapping their felt affect *sub-frame-by-sub-frame* over rich audio/video, paired with physiology (the protocol in [CONCEPTS.md §3](CONCEPTS.md)) — and even that only seeds the shadow richly; the pull itself is earned by living.
